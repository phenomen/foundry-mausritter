...
export async function createCharacter(options, additionalWeaponsItems) {
	...
	const characterStats = await createStats(
		background,
		options.stats,
		options.details,
	);
	...
	if (
		highestAttrValue <= TAKE_BOTH_ADDITIONAL_ITEMS &&
		options.items &&
		options.background
	) {
		const additionalItemsBackground =
			getRandomBackgroundDifferentThan(background);
		getBackgroundItems(additionalItemsBackground).then(async (items) => {
		...
		if (
			highestAttrValue <= TAKE_ONE_OF_ADDITIONAL_ITEMS &&
			options.items &&
			options.background
		) {
			const additionalItemsBackground =
			...
			getBackgroundItems(additionalItemsBackground).then(async (items) => {
				await showAdditionalItemsChoiceDialog(items, async (selectedIndex) => {
					await addItem(
						itemCompendiumIds[selectedIndex],
						characterActor,
						SLOTS.SLOT_2,
					);
				});
...
function getRandomBackgroundDifferentThan(current) {
	...
	return background.name === current.name
		? getRandomBackgroundDifferentThan(current)
		: background;
}
...
async function createStats(background, basicStats, details) {
	...
	const birthSign = details
		? await drawFromTable(CONFIG.MAUSRITTER.tables.birthsign)
		: "";
	const look = details
		? await drawFromTable(CONFIG.MAUSRITTER.tables.physicalDetail)
		: "";
	const coat = details
		? `${await drawFromTable(CONFIG.MAUSRITTER.tables.coatPattern)} ${await drawFromTable(CONFIG.MAUSRITTER.tables.coatColor)}`
		: "";
